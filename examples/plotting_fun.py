...
def plot_latent(encoder, dataset, posterior=(0, 1), scale_radius=1, n_sample=1000):
    ...
    for b in range(len(dataset)):
        circle = Ellipse(
            (latent_mean[b, 0], latent_mean[b, 1]),
...
def plot_reconstruction_with_latent(
    model, dataset, posterior=(0, 1), data_idx=0, scale_radius=1, n_sample=1000, apply_softmax=False
):
    mean, std_deviation = posterior
    plt.figure(figsize=(15, 3))
    ...
    circle = Ellipse(
        (latent_mean[data_idx, 0], latent_mean[data_idx, 1]),
...
def plot_std(std_deviation, encoder_size, decoder_size):
    ...
    plt.plot(
        np.arange(encoder_size, encoder_size + decoder_size),
        std_deviation[-decoder_size:],
        label="Decoder",
        alpha=1,
    )
    plt.yscale("log")
    ...
    plt.subplot(1, 2, 1)
    plt.plot(
        np.arange(encoder_size - 102, encoder_size),
        std_deviation[encoder_size - 102 : encoder_size],
        label="Encoder",
        alpha=1,
        c="C0",
    )
    ...
    plt.subplot(1, 2, 2)
    plt.plot(
        np.arange(encoder_size, encoder_size + 150),
        std_deviation[encoder_size : encoder_size + 150],
        label="Decoder",
        alpha=1,
        c="C1",
    )
